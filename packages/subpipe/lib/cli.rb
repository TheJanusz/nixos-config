...
COMMANDS = {
  ...
  "translate" => {
    ...
    summary: "Translate context.json cues to Polish and write {stem}.pl.ass; uses emotion tags when present.",
    needs: "merge (prefer: analyze first)"
  },
  "review" => {
    usage: "subpipe review (-o DIR | VIDEO) [--vocab PATH]",
    summary: "Browse cues one-by-one (j/k); edit PL/emotion/glossary; apply / promote vocab.",
    needs: "translate --mode draft (or context with text_pl)"
  },
  "run" => {
  ...
  "lektor" => {
    usage: "subpipe lektor <init|tui|generate> (-o DIR | VIDEO) …",
    summary: "Offline XTTS-v2 narrator: voice.json + reference.wav, preview TUI, batch cue WAVs.",
    needs: "translate (context.json with text_pl); analyze tags optional for delivery speed"
  }
}.freeze
...
LEKTOR_COMMANDS = {
  "init" => {
    ...
    summary: "Create voice.json; optionally copy a 6–30s mono reference.wav into the work dir."
  },
  "tui" => {
    usage: "subpipe lektor [tui] (-o DIR | VIDEO)",
    summary: "Browse cues (j/k), preview with XTTS, edit lektor_line, tweak/save voice profile."
  },
  "generate" => {
...
def usage!(code: 1)
  ...
  lines << "Lektor: SUBPIPE_XTTS_WORKER, SUBPIPE_XTTS_MODEL, SUBPIPE_XTTS_DEVICE, SUBPIPE_XTTS_HOOK"
  warn lines.join("\n")
  exit code
end
...
def lektor_usage!(code: 1, error: nil)
  ...
  lines << "subpipe lektor — offline XTTS-v2 narrator (reference.wav + voice.json)"
  lines << ""
  ...
  lines << ""
  lines << "Timbre comes from reference.wav (not a text prompt). Delivery uses speed + analyze tags."
...
when "review"
  ...
  OptionParser.new do |opts|
    ...
    opts.banner = <<~BANNER.chomp
      ...
      Requires: #{meta[:needs]}
      Pass -o DIR or VIDEO ({stem}.subpipe/).
      Single-cue browser: full wrapped EN/PL, j/k to move, 0/G first/last.
      Cue keys: e edit PL, m emotion, t toggle review, c clear flags; g glossary.
      Loads translation-draft.json (or builds one from context.json). No embedded video player;
      for narrator preview use: subpipe lektor -o DIR
    BANNER
    opts.on("-o DIR", "--out DIR", "Episode output directory") { |v| options[:out] = v; options[:out_set] = true }
    opts.on("--vocab=PATH", "--vocab PATH", "Show-level vocab.json (relative ok)") { |v| options[:vocab] = v }
    opts.on("-h", "--help", "Show help") { puts opts; exit 0 }
  ...
  Subpipe::Review.run(out, vocab_path: options[:vocab])
...
when "lektor"
  sub = ARGV.shift
  # Bare `subpipe lektor -o DIR` → tui; also accept tui|preview|init|generate
  if sub.nil? || %w[-h --help help].include?(sub)
    lektor_usage!(code: 0)
  end

  mode =
    if %w[init tui preview generate].include?(sub)
      sub == "preview" ? "tui" : sub
  ...
  options = { out: nil, out_set: false, force: false, reference: nil }
  OptionParser.new do |opts|
    ...
    opts.banner = <<~BANNER.chomp
      ...
      #{meta[:summary]}
      Pass -o DIR or VIDEO ({stem}.subpipe/).
      voice.json knobs: speed, language, reference_wav, delivery_defaults.
      Timbre = reference.wav (6–30s clean speech). XTTS loads once per generate/TUI session.
      Env: SUBPIPE_XTTS_WORKER, SUBPIPE_XTTS_MODEL, SUBPIPE_XTTS_DEVICE (cuda|cpu), SUBPIPE_XTTS_HOOK
    BANNER
    opts.on("-o DIR", "--out DIR", "Episode output directory") { |v| options[:out] = v; options[:out_set] = true }
    opts.on("--reference WAV", "Copy reference clip into work dir (init)") { |v| options[:reference] = v }
    opts.on("--force", "Regenerate cue WAVs even when fingerprint matches") { options[:force] = true }
    opts.on("-h", "--help", "Show help") { puts opts; exit 0 }
  end.parse!
